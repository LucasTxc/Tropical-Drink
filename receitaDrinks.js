// TRANFOMAÇÃO PARA JS
export let drinks = [
    {
        titulo: "Maracuja",
        imagem: "./ASSETS/M1.png",
        ingredientes: [
            "2 oz Cachaça",
            "1/2 Maracuja",
            "2 colheres de bar de Açúcar",
            "8 cubos de gelo"]
    },

    {
        titulo: "Laranja",
        imagem: "./ASSETS/L2.png",
        ingredientes: [
            "50ml de vodka",
            "50ml de suco de laranja",
            "Raspas de limão",
            "8 cubos de gelo"]

    },
    {
        titulo: "Morango",
        imagem: "./ASSETS/Mo3.png",
        ingredientes: [
            "1 xícara (chá) de Morangos (picados)",
            "1 colher (sopa) de Mel",
            "½ xícara (chá) de Suco de Laranja Natural",
            "1 litro de Refrigerante de Limão"]
    },
    {
        titulo: "Limão",
        imagem: "./ASSETS/limão.png",
        ingredientes: [
            "60 ml de cachaça",
            "1 limão",
            "2 colheres (chá) de açúcar (ou 3 cubos)"
        ]
    },
    {
        titulo: "Abacaxi",
        imagem: "./ASSETS/abacaxi.png",
        ingredientes: [
            "1 abacaxi pequeno",
            "1 copo de suco de laranja",
            "1½ copos de vodka",
            " folhas de hortelã",
            "pedras de gelo"
        ]
    },
    {
        titulo: "Uva",
        imagem: "./ASSETS/uva.png",
        ingredientes: [
            "20 unidades de uva verde",
            "4 colheres (sopa) de açúcar",
            "4 unidades de gelo",
            "2 doses de cachaça"]
    },
    {
        titulo: "Sex on beach",
        imagem: "./ASSETS/sexOnthebeach.png",
        ingredientes: [
            "2 doses de vodka",
            "1 dose de suco de pêssego",
            "2 doses de suco de laranja",
            "groselha"]
    },

    {
        titulo: "Mojito",
        imagem: "./ASSETS/mojito.png",
        ingredientes: [
            "1 limão",
            "1 dose de rum branco (50ml)",
            "3 ramos de hortelã",
            "cubos de gelo a gosto",
            "club soda ou água com gás para"

        ]
    },
    {
        titulo: "Pina Colada",
        imagem: "./ASSETS/pinacolada.png",
        ingredientes: [
            "1 dose de rum (50 ml)",
            "1 dose de leite de coco (50 ml)",
            "2 doses de suco de abacaxi (100 ml)",
            "1 colher (sopa) de açúcar",
            "cubos de gelo"
        ]
    },
    {
        titulo: "Marguarita",
        imagem: "./ASSETS/marguarita.png",
        ingredientes: [
            "40 ml de tequila",
            "20 ml de cointreau",
            "2 colheres (sopa) de suco de limão raspas de 1 limão pequeno"
        ]
    },

    {
        titulo: "Aperol",
        imagem: "./ASSETS/aperol.png",
        ingredientes: [
            "3 partes de prosecco)",
            "2 partes de Aperol",
            "1 partes de água com gás",
            "Gelo a gosto",
            "Fatia de laranja para decorar"]
    },

    {
        titulo: "Cosmopolitan",
        imagem: "./ASSETS/cosmopolitan.png",
        ingredientes: [
            "1 dose de vodka",
            "1 limão (suco de)",
            "50ml suco de cranberry",
            "1 dose de licor de laranja",
            "gelo"]
    }

]





